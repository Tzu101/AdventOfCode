...
fn get_trailhead_score(trail_map: &Vec<Vec<char>>, row: usize, col: usize, height: char, unique_ends: &mut Vec::<bool>) -> u32 {
    ...
    if row > 0 && is_ascending(trail_map[row][col], trail_map[row - 1][col]) {
        score += get_trailhead_score(trail_map, row - 1, col, height, unique_ends);
    }
    ...
    if col > 0 && is_ascending(trail_map[row][col], trail_map[row][col - 1]) {
        score += get_trailhead_score(trail_map, row, col - 1, height, unique_ends);
    }
...
pub fn part1() -> String {
    ...
    for row in 0..trail_map.len() {
        for col in 0..trail_map[0].len() {
            ...
            trailhead_score += get_trailhead_score(&trail_map, row, col, '0', &mut unique_ends);
        }
    }
